...
import 'package:supplier/model/kategoi_model.dart' as kat;
import 'package:supplier/model/master_model.dart';

String token = "";
String images = "";

class UpdateBarangPage extends StatelessWidget {
  const UpdateBarangPage({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    ], child: const UpdateBarangContent());
  }
}

class UpdateBarangContent extends StatefulWidget {
  const UpdateBarangContent({Key? key}) : super(key: key);

  @override
  State<UpdateBarangContent> createState() => _UpdateBarangContentState();
}

class _UpdateBarangContentState extends State<UpdateBarangContent> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (args != null) {
        ...
        var rows = args['rows'];
        await context.read<_ProviderGetData>().setData(rows);
        var res = context.read<_ProviderGetData>().result;
        if (res != null) {
          _namaBarangController.text = res.nmbarang;
          _hargaBeliController.text = res.hbeli.toString();
          _hargaJualController.text = res.hjual.toString();

          var kat = context.read<_ProviderGetKategori>().result;
          for (var item in kat) {
            if (item.nmkat == res.nmkat) {
              context.read<_ProviderGetKategori>().setSelectKategori(item);
              break;
            }
          }
          images = res.image;
                      ...
                      builder: (context, value, child) {
                        ...
                        if (barang.isEmpty) {
                          ...
                          return Container(
                            ...
                            child: DropdownButton<kat.Result>(
                              ...
                              items: barang.map((kat.Result result) {
                                return DropdownMenuItem<kat.Result>(
                                  value: result,
                                  child: Text(
                                    result.nmkat,
                                    textScaleFactor: 1,
              ...
              Expanded(
                child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      _updateApi(context, {
                        "idbarang":
                            context.read<_ProviderGetData>().result!.idbarang,
                        "nmbarang": _namaBarangController.text.trim(),
                        "hjual": _hargaJualController.text.trim(),
                        "hbeli": _hargaBeliController.text.trim(),
                        "idkat": context
...
Future<void> _updateApi(BuildContext context, dynamic params) async {
  print(params);
  int act = 3;
  AppDialogs.showProgressDialog(
      context: context, title: "Prosess", message: "Proses ubah data");
  ...
  if (response.data['result'] != null) {
    await AppDialogs.resultDialog(
        context: context, title: "Berhasil", message: "Data berhasil diubah");
    Navigator.pop(context);
  }
}
...
class _ProviderGetKategori with ChangeNotifier {
  List<kat.Result> result = [];
  kat.Result? selectKategori;
  int act = 1;
  ...
  Future<void> setData(BuildContext context, {bool append = false}) async {
    ...
    if (response.data != null) {
      kat.ModelKategori model = kat.ModelKategori.fromJson(response.data);
      result.clear();
  ...
  void setSelectKategori(kat.Result result) {
